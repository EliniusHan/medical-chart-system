...
# ============================================
# 검사결과 단위/참고범위 자동 채우기 매핑
# (검사항목: (단위, 참고범위_남, 참고범위_여)  여=None이면 남녀 동일)
# ============================================
_검사_참고정보 = {
    "WBC":              ("10³/uL",       "4.0-10.5",  None),
    "Hgb":              ("g/dL",         "13.0-17.0", "12.0-16.0"),
    "Hb":               ("g/dL",         "13.0-17.0", "12.0-16.0"),
    "Plt":              ("10³/uL",       "150-400",   None),
    "RBC":              ("10⁶/uL",       "4.5-5.5",   "4.0-5.0"),
    "Hct":              ("%",            "39-51",     "36-47"),
    "AST":              ("U/L",          "0-40",      None),
    "ALT":              ("U/L",          "0-40",      None),
    "GGT":              ("U/L",          "11-63",     "8-35"),
    "ALP":              ("U/L",          "40-130",    None),
    "Total bilirubin":  ("mg/dL",        "0.1-1.2",   None),
    "Albumin":          ("g/dL",         "3.5-5.2",   None),
    "BUN":              ("mg/dL",        "7-20",      None),
    "Creatinine":       ("mg/dL",        "0.7-1.3",   "0.6-1.1"),
    "Cr":               ("mg/dL",        "0.7-1.3",   "0.6-1.1"),
    "eGFR":             ("mL/min/1.73m²", ">60",      None),
    "Na":               ("mEq/L",        "136-145",   None),
    "K":                ("mEq/L",        "3.5-5.0",   None),
    "Cl":               ("mEq/L",        "98-106",    None),
    "Ca":               ("mg/dL",        "8.6-10.2",  None),
    "TC":               ("mg/dL",        "<200",      None),
    "Total Cholesterol":("mg/dL",        "<200",      None),
    "LDL":              ("mg/dL",        "<130",      None),
    "HDL":              ("mg/dL",        ">40",       ">50"),
    "TG":               ("mg/dL",        "<150",      None),
    "Triglyceride":     ("mg/dL",        "<150",      None),
    "FBS":              ("mg/dL",        "70-99",     None),
    "Fasting Blood Sugar": ("mg/dL",     "70-99",     None),
    "HbA1c":            ("%",            "4.0-5.6",   None),
    "A1c":              ("%",            "4.0-5.6",   None),
    "PP2":              ("mg/dL",        "<140",      None),
    "TSH":              ("mIU/L",        "0.27-4.20", None),
    "fT4":              ("ng/dL",        "0.93-1.70", None),
    "T3":               ("ng/dL",        "80-200",    None),
    "Uric acid":        ("mg/dL",        "3.4-7.0",   "2.4-6.0"),
    "UA":               ("mg/dL",        "3.4-7.0",   "2.4-6.0"),
    "CRP":              ("mg/dL",        "<0.5",      None),
    "ESR":              ("mm/hr",        "0-15",      "0-20"),
    "microalbumin":     ("mg/L",         "<20",       None),
}
...
def DB연결():
    ...
    기존_칼럼_추가 = [
        ...
        ("진단",    "정정사유",  "TEXT"),
        ("검사결과", "방문id",   "INTEGER REFERENCES 방문(방문id)"),
        ("검사결과", "결과수치", "REAL"),
        ("검사결과", "유효여부", "INTEGER DEFAULT 1"),
        ("검사결과", "정정사유", "TEXT"),
        ("영상검사", "방문id",   "INTEGER REFERENCES 방문(방문id)"),
    ...
    # 기존 검사결과 결과수치 1회성 마이그레이션 (NULL인 행만 대상)
    conn.execute("""
        UPDATE 검사결과
        SET 결과수치 = CAST(결과값 AS REAL)
        WHERE 결과수치 IS NULL
          AND typeof(CAST(결과값 AS REAL)) = 'real'
          AND CAST(결과값 AS REAL) != 0
    """)
...
def 검사결과추가(환자id, 검사시행일, 검사항목, 결과값, 단위="", 참고범위="", 방문id=None):
    """혈액검사 결과를 추가한다. 단위/참고범위가 비어있으면 자동으로 채운다."""
    try:
        결과수치 = float(결과값)
    except (ValueError, TypeError):
        결과수치 = None

    # 단위/참고범위 자동 채우기 (대소문자 무시 + 공백 제거 매칭)
    if not 단위 or not 참고범위:
        정규화키 = (검사항목 or "").strip().lower()
        참고 = None
        for k, v in _검사_참고정보.items():
            if k.lower() == 정규화키:
                참고 = v
                break
        if 참고:
            ...
            if not 참고범위:
                if 참고여:
                    _conn = DB연결()
                    try:
                        row = _conn.execute("SELECT 성별 FROM 환자 WHERE 환자id=?", (환자id,)).fetchone()
                        성별 = row[0] if row else ""
                        참고범위 = 참고여 if 성별 == "F" else 참고남
                    finally:
                        _conn.close()
                else:
                    참고범위 = 참고남
    ...
    try:
        with conn:
            conn.execute(
                """INSERT INTO 검사결과
                   (환자id, 방문id, 검사시행일, 검사항목, 결과값, 결과수치, 단위, 참고범위, 유효여부)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)""",
                (환자id, 방문id, 검사시행일, 검사항목, 결과값, 결과수치, 단위, 참고범위)
            )
    finally:
        conn.close()


def 영상검사추가(환자id, 검사시행일, 검사종류, 결과요약="", 주요수치="", 방문id=None):
    """영상검사 결과를 추가한다."""
    conn = DB연결()
    try:
        with conn:
            conn.execute(
                """INSERT INTO 영상검사
                   (환자id, 방문id, 검사시행일, 검사종류, 결과요약, 주요수치, 유효여부)
                   VALUES (?, ?, ?, ?, ?, ?, 1)""",
                (환자id, 방문id, 검사시행일, 검사종류, 결과요약, 주요수치)
            )
    finally:
        conn.close()
...
def 분석완료처리(방문id):
    ...
    try:
        ...
        with conn:
            conn.execute(
                "UPDATE 방문 SET 분석완료 = 1 WHERE 환자id = ? AND 방문일 = ? AND 유효여부 = 1",
                (원본["환자id"], 원본["방문일"])
            )
    finally:
        conn.close()
...
def 방문기록_일괄수정(방문id, 필드dict):
    """AI 분석 최초 저장 시 방문 테이블의 여러 필드를 한 번에 UPDATE.
...
def 환자전체기록조회(환자id, 외부기록_제외=False):
    ...
    try:
        ...
        검사목록 = conn.execute(
            """SELECT 검사id, 환자id, 방문id, 검사시행일, 검사항목,
                      결과값, 결과수치, 단위, 참고범위, 유효여부, 정정사유
               FROM 검사결과 WHERE 환자id = ? AND 유효여부 = 1
               ORDER BY 검사시행일 DESC""",
            (환자id,)
        ).fetchall()

        영상목록 = conn.execute(
            """SELECT 영상id, 환자id, 방문id, 검사시행일, 검사종류,
                      결과요약, 주요수치, 유효여부, 정정사유
               FROM 영상검사 WHERE 환자id = ? AND 유효여부 = 1
               ORDER BY 검사시행일 DESC""",
            (환자id,)
        ).fetchall()
...
def 검사결과수정(검사id, 수정할항목, 새값, 정정사유=""):
    ...
    try:
        ...
        with conn:
            ...
            cursor = conn.execute(
                """INSERT INTO 검사결과
                   (환자id, 검사시행일, 검사항목, 결과값, 결과수치, 단위, 참고범위, 유효여부)
                   VALUES (?, ?, ?, ?, ?, ?, ?, 1)""",
                (새레코드['환자id'], 새레코드['검사시행일'], 새레코드['검사항목'],
                 새레코드['결과값'], 새레코드.get('결과수치'), 새레코드['단위'], 새레코드['참고범위'])
            )
            return cursor.lastrowid
    finally:
        conn.close()
...
# ============================================
def 환자삭제(환자id):
    """해당 환자의 모든 기록(8개 테이블)을 실제 삭제한다. (테스트 데이터 정리용)"""
    conn = DB연결()
    try:
        with conn:
...
def 방문기록삭제(방문id, 삭제사유=""):
    """방문을 무효화한다. 연결된 진단, 추적계획, 처방, 검사처방, 검사결과, 영상검사도 함께 무효화."""
    conn = DB연결()
    try:
        with conn:
            conn.execute(
            ...
            conn.execute(
                "UPDATE 처방 SET 유효여부 = 0, 정정사유 = ? WHERE 방문id = ? AND 유효여부 = 1",
                (삭제사유, 방문id)
            )
            conn.execute(
                "UPDATE 검사처방 SET 유효여부 = 0, 정정사유 = ? WHERE 방문id = ? AND 유효여부 = 1",
                (삭제사유, 방문id)
            )
            conn.execute(
