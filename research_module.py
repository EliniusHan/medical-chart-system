...
- 검사결과(검사id, 환자id, 방문id, 검사시행일, 검사항목, 결과값, 결과수치, 단위, 참고범위)
- 영상검사(영상id, 환자id, 방문id, 검사시행일, 검사종류, 결과요약, 주요수치)
...
- 유효여부 필터 규칙:
  환자 테이블에는 유효여부 칼럼이 없음! 환자 테이블에 유효여부 조건을 걸지 마세요.
  유효여부가 있는 테이블: 방문, 진단, 검사결과, 영상검사, 추적계획, 처방, 검사처방
  이 테이블들만 유효여부=1 조건을 적용하세요.
  올바른 예: WHERE d.유효여부 = 1 AND v.유효여부 = 1
  잘못된 예: WHERE p.유효여부 = 1  ← 환자 테이블에는 유효여부 없음! ❌"""
...
중요: 유효여부 필터 규칙을 반드시 준수하세요.
환자 테이블에는 유효여부 칼럼이 없으므로 환자 테이블에 유효여부 조건을 걸지 마세요.
유효여부가 있는 테이블(방문, 진단, 검사결과, 영상검사, 추적계획, 처방, 검사처방)에만 유효여부=1을 적용하세요.
올바른 예: WHERE d.유효여부 = 1 AND v.유효여부 = 1
잘못된 예: WHERE p.유효여부 = 1  ← 환자 테이블에는 유효여부 없음! ❌"""
...
def _run_stat_code(code):
    ...
    # 타임스탬프 + 랜덤 suffix 파일명 (충돌 방지)
    graph_path = os.path.join(OUTPUT_DIR, f"analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:4]}.png")
...
def _복수방법_감지(ai_제안):
    """AI 제안 텍스트에 번호 목록이 2개 이상 있으면 복수 방법 제안으로 판단."""
    번호패턴 = re.findall(r'(?:^|\n)\s*[1-9][.)\s]', ai_제안)
    return len(번호패턴) >= 2


def _단계_확인(ai_제안, 단계명, 프롬프트="진행(y) / 수정 사항을 자유롭게 입력하세요(재분석): "):
...
def 통계분석_단계별():
    ...
    # Step 2: 그룹 설정
    print("\n Step 2: 그룹 설정 중...")
    제안 = _ai_제안(대화기록, "그룹 설정 방법을 번호와 함께 추천해주세요.")
    while True:
        print(f"\n [AI 제안 — 그룹 설정]\n{제안}\n")
        응답 = input(" 번호 선택 / 수정 사항을 자유롭게 입력하세요(재분석): ").strip()
        if 응답.isdigit():
            제안 = _ai_제안(대화기록, f"{응답}번 그룹 설정으로 확정합니다. 다음 단계로 넘어가세요.")
            break
        else:
    ...
    while True:
        ...
        if _복수방법_감지(제안):
            # 복수 방법 제안 → 선택지 풍부하게
            print(" ※ 여러 분석 방법이 제안되었습니다.")
            응답 = input(
                " 번호 선택(하나만) / 조합(여러 개 조합) / AI에게 맡기기(y) / 직접 선택(m)"
                " / 수정 사항을 자유롭게 입력하세요(재분석): "
            ).strip()

            if 응답.lower() == "y":
                # AI가 알아서 최적 조합으로 진행
                제안 = _ai_제안(대화기록,
                    "제안된 방법들 중 가장 적합한 방법(들)을 선택하여 진행해주세요. "
                    "필요하면 여러 방법을 조합하세요.")
                break
            elif 응답.lower() in ("m", "직접선택"):
                print(통계방법_목록)
                번호 = input(" 번호를 선택하세요: ").strip()
                if 번호.isdigit() and 1 <= int(번호) <= 12:
                    선택방법 = 통계방법_목록.split("\n")[int(번호)].strip()
                    제안 = _ai_제안(대화기록,
                        f"통계 방법 '{선택방법}'을 사용하는 방향으로 분석 계획을 수정해주세요.")
                else:
                    print(" 올바른 번호를 입력해주세요.")
            elif 응답.lower() == "조합":
                조합내용 = input(
                    " 조합할 방법 번호 또는 내용을 입력하세요 (예: 1, 3 / 기술통계 + 상관분석): "
                ).strip()
                제안 = _ai_제안(대화기록,
                    f"다음 방법들을 조합하여 분석 계획을 구성해주세요: {조합내용}")
            elif 응답.isdigit():
                제안 = _ai_제안(대화기록,
                    f"{응답}번 방법으로 분석을 진행해주세요.")
                break
            else:
                제안 = _ai_제안(대화기록,
                    f"수정 요청: {응답}. 통계 기법을 재추천해주세요.")

        else:
            # 단일 방법 제안 → 간결하게
            응답 = input(
                " AI 추천 사항으로 진행(y) / 직접 선택(m)"
                " / 수정 사항을 자유롭게 입력하세요(재분석): "
            ).strip()
