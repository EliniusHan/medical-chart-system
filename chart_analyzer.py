...
검사결과 단위/참고범위 자동 채우기 규칙:

  의사가 단위나 참고범위를 차트에 적지 않는 경우가 많습니다.
  아래 일반적인 검사 항목은 단위와 참고범위를 자동으로 채워주세요.

  혈액검사 (CBC):
    WBC: 단위=10³/uL, 참고범위=4.0-10.5
    Hgb (또는 Hb): 단위=g/dL, 참고범위=남 13.0-17.0 / 여 12.0-16.0
    Plt: 단위=10³/uL, 참고범위=150-400
    RBC: 단위=10⁶/uL, 참고범위=남 4.5-5.5 / 여 4.0-5.0
    Hct: 단위=%, 참고범위=남 39-51 / 여 36-47

  간기능 (LFT):
    AST: 단위=U/L, 참고범위=0-40
    ALT: 단위=U/L, 참고범위=0-40
    GGT: 단위=U/L, 참고범위=남 11-63 / 여 8-35
    ALP: 단위=U/L, 참고범위=40-130
    Total bilirubin: 단위=mg/dL, 참고범위=0.1-1.2
    Albumin: 단위=g/dL, 참고범위=3.5-5.2

  신장기능:
    BUN: 단위=mg/dL, 참고범위=7-20
    Creatinine (Cr): 단위=mg/dL, 참고범위=남 0.7-1.3 / 여 0.6-1.1
    eGFR: 단위=mL/min/1.73m², 참고범위=>60

  전해질:
    Na: 단위=mEq/L, 참고범위=136-145
    K: 단위=mEq/L, 참고범위=3.5-5.0
    Cl: 단위=mEq/L, 참고범위=98-106
    Ca: 단위=mg/dL, 참고범위=8.6-10.2

  지질 (Lipid):
    TC (Total Cholesterol): 단위=mg/dL, 참고범위=<200
    LDL: 단위=mg/dL, 참고범위=<130 (고위험군 <100, 초고위험군 <70)
    HDL: 단위=mg/dL, 참고범위=남 >40 / 여 >50
    TG (Triglyceride): 단위=mg/dL, 참고범위=<150

  당뇨:
    FBS (Fasting Blood Sugar): 단위=mg/dL, 참고범위=70-99
    HbA1c (A1c): 단위=%, 참고범위=4.0-5.6
    PP2 (식후 2시간 혈당): 단위=mg/dL, 참고범위=<140

  갑상선:
    TSH: 단위=mIU/L, 참고범위=0.27-4.20
    fT4: 단위=ng/dL, 참고범위=0.93-1.70
    T3: 단위=ng/dL, 참고범위=80-200

  요산:
    Uric acid (UA): 단위=mg/dL, 참고범위=남 3.4-7.0 / 여 2.4-6.0

  소변검사:
    Urine protein: 단위=정성, 참고범위=negative
    Urine glucose: 단위=정성, 참고범위=negative
    microalbumin (소변): 단위=mg/L, 참고범위=<20

  염증:
    CRP: 단위=mg/dL, 참고범위=<0.5
    ESR: 단위=mm/hr, 참고범위=남 0-15 / 여 0-20

  환자 성별에 따라 남/여 참고범위를 구분해서 적용하세요.
  위 목록에 없는 검사 항목이라도, 의학적으로 잘 알려진 검사라면
  일반적인 단위와 참고범위를 채워주세요.
  확실하지 않은 경우에만 비워두세요.
"""
...
def 분석결과_저장(환자id, 방문id, final_free_text, approved_data):
    ...
    for k in approved_data.get("lab_results", []):
        시행일_k = k.get("검사시행일", "")
        대상방문id = _방문id_결정(환자id, 시행일_k, 방문id)
        검사결과추가(환자id, 시행일_k, k["검사항목"], k.get("결과값", ""),
                   k.get("단위", ""), k.get("참고범위", ""), 방문id=대상방문id)
        _검사처방_시행완료_매칭(환자id, 시행일_k, k["검사항목"])
        저장건수 += 1
        print(f"  → 검사결과 저장: {k['검사항목']} {k.get('결과값', '')} (방문id={대상방문id})")

    # --- 영상검사 ---
    for e in approved_data.get("imaging", []):
        시행일_e = e.get("검사시행일", "")
        대상방문id = _방문id_결정(환자id, 시행일_e, 방문id)
        영상검사추가(환자id, 시행일_e, e["검사종류"], e.get("결과요약", ""),
                   e.get("주요수치", ""), 방문id=대상방문id)
        저장건수 += 1
        print(f"  → 영상검사 저장: {e['검사종류']} (방문id={대상방문id})")
...
# ============================================================
def _날짜_YYMMDD(s):
    """어떤 형식이든 YYMMDD(6자리)로 정규화. 빈 값은 빈 문자열."""
    s = str(s or "").replace("-", "")
    if len(s) == 8:       # YYYYMMDD → YYMMDD
        return s[2:]
    return s              # YYMMDD, YYMM00, 그 외는 그대로


def _방문id_결정(환자id, 검사시행일, 현재방문id):
    """검사결과/영상검사 저장 시 적절한 방문id를 반환.

    규칙:
      (a) 검사시행일 없음 → 현재방문id
      (b) 검사시행일 == 현재 방문일 → 현재방문id (당일 검사)
      (c) 같은 날짜의 유효 방문 존재 → 그 방문id
      (d) 없으면 빈 방문 레코드 생성 후 그 방문id (외부 검사 기록용)

    (d)의 가상 방문: free_text='', 분석완료=1, 유효여부=1.
    미분석 차트 조회에 뜨지 않도록 분석완료=1로 저장.
    """
    시행일_정규 = _날짜_YYMMDD(검사시행일)
    if not 시행일_정규:
        return 현재방문id
    ...
    try:
        # (b) 현재 방문일 확인 — 같은 날이면 현재 방문 재사용
        현재 = conn.execute(
            "SELECT 방문일 FROM 방문 WHERE 방문id=?", (현재방문id,)
        ).fetchone()
        if 현재 and _날짜_YYMMDD(현재["방문일"]) == 시행일_정규:
            return 현재방문id

        # (c) 같은 시행일에 해당하는 유효 방문 찾기
        rows = conn.execute(
            "SELECT 방문id, 방문일 FROM 방문 WHERE 환자id=? AND 유효여부=1 "
            "ORDER BY 방문id DESC",
            (환자id,)
        ).fetchall()
        for r in rows:
            if _날짜_YYMMDD(r["방문일"]) == 시행일_정규:
                return r["방문id"]
    finally:
        conn.close()
...
def _검사처방_시행완료_매칭(환자id, 검사시행일, 검사항목):
    ...
    날짜가 일치하는 처방건만 매칭 (다른 날짜 처방은 건드리지 않음)."""
    시행일_비교 = _날짜_YYMMDD(검사시행일)
    if not 시행일_비교 or not 검사항목:
        return

    conn = sqlite3.connect(DB경로)
    try:
        미시행 = conn.execute(
            "SELECT 처방검사id, 검사명, 처방일 FROM 검사처방 "
            "WHERE 환자id=? AND 시행여부=0 AND 유효여부=1",
            (환자id,)
        ).fetchall()

        for 처방검사id, 검사명, 처방일 in 미시행:
            # 양쪽 날짜 모두 정규화해서 비교
            if _날짜_YYMMDD(처방일) != 시행일_비교:
                continue
            검사명 = 검사명 or ""
...
의사 차트 스타일 규칙:
  ** (2개) = 계획 항목
  ...
  [날짜 없는 **] — 방문일에 시행/처방한 것
    **약 N개월           → 처방 (일수=N×30)
    **Lab(검사명)        → 검사처방: 검사명="Lab (기본패널 + 괄호 안 항목)", 처방일=방문일
    **영상검사명 (CXR, EKG, 초음파 등)
                        → 검사처방: 검사명=해당 검사, 처방일=방문일
                        → 결과 수치/소견이 없으므로 검사결과/영상검사에 넣지 말 것!

  [날짜 있는 **] — 해당 월에 할 예정
    **YYMM Lab(검사명)   → 검사처방(처방일=YYMM00) + 추적계획(예정일=YYMM00)
    **YYMM 검사명         → 검사처방(처방일=YYMM00) + 추적계획(예정일=YYMM00)
    **YYMM 약 N개월      → 처방(일수=N×30) + 추적계획(예정일=YYMM00)

  예시:
    "**CXR"              → 추가: {"테이블": "검사처방", "데이터": {"검사명": "CXR", "처방일": "<방문일>"}}
    "**경동맥 초음파"      → 추가: {"테이블": "검사처방", "데이터": {"검사명": "경동맥 초음파", "처방일": "<방문일>"}}
    "**2607 Chest CT"    → 추가: {"테이블": "검사처방", "데이터": {"검사명": "Chest CT", "처방일": "260700"}}
                           추가: {"테이블": "추적계획", "데이터": {"예정일": "260700", "내용": "Chest CT"}}
    "CXR 정상"           → 추가: {"테이블": "영상검사", "데이터": {"검사종류": "CXR", "결과요약": "정상", "검사시행일": "<방문일>"}}
    "A1c 6.5"            → 추가: {"테이블": "검사결과", "데이터": {"검사항목": "A1c", "결과값": "6.5", "검사시행일": "<방문일>"}}
    "260301 A1c 6.5"     → 추가: {"테이블": "검사결과", "데이터": {"검사항목": "A1c", "결과값": "6.5", "검사시행일": "260301"}}

진단명 표기:
  #으로 시작 → 진단명 (예: #고혈압 → {"테이블": "진단", "데이터": {"진단명": "고혈압", "상태": "활성"}})
  #r/o, #의심 → 상태='의심'
  괄호 안 내용 → 비고
...
=== JSON 출력 형식 ===
{
  ...
  "추가": [
    ...
    {"테이블": "검사처방",  "데이터": {"검사명": "CXR", "처방일": "260315"}},
    {"테이블": "영상검사",  "데이터": {"검사종류": "CXR", "결과요약": "정상", "검사시행일": "260301"}},
    {"테이블": "처방",      "데이터": {"약품명": "rosuvastatin", "용량": "10mg", "용법": "qd", "일수": 60}},
    {"테이블": "진단",      "데이터": {"진단명": "고혈압", "상태": "활성", "비고": ""}},
    {"테이블": "추적계획",  "데이터": {"예정일": "260700", "내용": "Chest CT"}}
  ],
  "삭제": [
...
변경 없는 항목은 빈 배열로. 활력징후_변경, 처방요약_변경은 변경 없으면 null로.
순수 JSON만 출력. 마크다운이나 설명 없이.

=== 날짜 저장 규칙 ===
  YYMMDD (6자리) → 그대로 저장. 예: 260315
  YYMM (4자리) → YYMM00으로 변환. 예: 2602 → 260200
  날짜 없으면 → 방문일 사용
  절대 4자리를 그대로 저장하지 말 것!
  같은 차트에서 같은 날짜에 시행된 검사는 모두 동일한 시행일로 저장.

=== 검사결과 단위/참고범위 자동 채우기 ===
검사결과 추가 시, 단위와 참고범위도 함께 채우세요.
의사가 단위를 적지 않아도 일반적으로 알려진 검사 항목이면 의학 지식으로 채워주세요.
예: FBS 80 → 검사항목=FBS, 결과값=80, 단위=mg/dL, 참고범위=70-99
예: A1c 5.5 → 검사항목=A1c, 결과값=5.5, 단위=%, 참고범위=4.0-5.6
확실하지 않은 경우에만 비워두세요."""
    ...
    try:
        ...
        for 항목 in 변경사항.get("변경", []):
            ...
            if 테이블 == "검사결과":
                ...
                conn.commit()
                시행일_수정 = 수정후.get("검사시행일", 시행일)
                대상방문id = _방문id_결정(환자id, 시행일_수정, 방문id)
                검사결과추가(환자id, 시행일_수정,
        ...
        for 항목 in 변경사항.get("추가", []):
            ...
            if 테이블 == "검사결과":
                시행일_추가 = 데이터.get("검사시행일", 방문일)
                대상방문id = _방문id_결정(환자id, 시행일_추가, 방문id)
                검사결과추가(환자id, 시행일_추가,
                           데이터.get("검사항목"), 데이터.get("결과값", ""),
                           데이터.get("단위", ""), 데이터.get("참고범위", ""),
                           방문id=대상방문id)
                _검사처방_시행완료_매칭(환자id, 시행일_추가, 데이터.get("검사항목", ""))
                변경건수 += 1
...
def 차트_재분석_저장(환자id, 방문id, 새_free_text, 방문일, 정정사유="재분석"):
    ...
    try:
        for 테이블 in ["진단", "추적계획", "처방", "검사처방"]:
            conn.execute(
                f"UPDATE {테이블} SET 유효여부=0, 정정사유=? WHERE 방문id=? AND 유효여부=1",
                (무효화사유, 방문id)
            )
        conn.execute(
            "UPDATE 검사결과 SET 유효여부=0, 정정사유=? WHERE 방문id=? AND 유효여부=1",
            (무효화사유, 방문id)
        )
        conn.execute(
            "UPDATE 영상검사 SET 유효여부=0, 정정사유=? WHERE 방문id=? AND 유효여부=1",
            (무효화사유, 방문id)
        )
        conn.commit()
    finally:
