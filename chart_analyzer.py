...
def _검사처방_시행완료_매칭(환자id, 검사시행일, 검사항목):
    ...
    try:
        ...
        # 검사시행일 정규화 (YYMMDD 6자리)
        시행일_비교 = str(검사시행일).replace("-", "")
        if len(시행일_비교) == 10:  # YYYY-MM-DD 형식
            시행일_비교 = 시행일_비교[2:]
